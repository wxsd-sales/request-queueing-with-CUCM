...
import com.ciscowebex.androidsdk.kitchensink.WebviewActivity
...
import com.ciscowebex.androidsdk.kitchensink.extras.ExtrasViewModel
...
//        Toast.makeText(requireActivity().applicationContext, "isUSMEnabled ${webexViewModel.webex.phone.isUnifiedSpaceMeetingEnabled()}", Toast.LENGTH_LONG).show()
    ...
    private fun setUpObservers() {
        val observer: Observer<String?> = Observer {
            accessToken = it?: getString(R.string.no_access_token_yet)
            Log.e("accessToken", accessToken)
        }

        extrasViewModel.accessToken.observe(viewLifecycleOwner!!, observer)
    }
    ...
    override fun onConnected(call: Call?) {
        ...
        onCallConnected(call?.getCallId().orEmpty(), call?.isCUCMCall() ?: false)
    ...
    override fun onClick(v: View?) {
        webexViewModel.currentCallId?.let { callId ->
            when (v) {
                ...
                binding.ibChat -> {
                    Log.e("accessToken", accessToken)
                    ContextCompat.startActivity(activity!!, WebviewActivity.getIntent(activity!!,accessToken,"25a9dcc6-c8d3-3b8f-a402-13c65b62e519@appid.ciscospark.com"), null)
//                    val intent = Intent (getActivity(),
//                        getActivity()?.let { WebviewActivity.getIntent(it,accessToken,spaceId) }, null)
//                    getActivity()?.startActivity(intent)
    ...
    private fun onCallConnected(callId: String, isCucmCall: Boolean) {
        Log.d(TAG, "CallControlsFragment onCallConnected callerId: $callId, currentCallId: ${webexViewModel.currentCallId}")
        if (webexViewModel.currentCallId.isNullOrEmpty() && isCucmCall) {
